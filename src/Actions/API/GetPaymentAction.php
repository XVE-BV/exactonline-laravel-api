...
{
    /**
     * Retrieve a single payment from Exact Online
     *
     * @param  ExactConnection  $connection  The Exact Online connection
     * @param  string  $paymentId  The payment ID (GUID)
     * @param  array{
     *     select?: array<string>|null,
     *     expand?: array<string>|null
     * }  $options  OData query options
     * @return array<string, mixed>|null Returns payment data or null if not found
     *
     * @throws ConnectionException
     */
    public function execute(ExactConnection $connection, string $paymentId, array $options = []): ?array
    {
        ...
        try {
            ...
            Log::info('Retrieved payment from Exact Online', [
                'connection_id' => $connection->id,
                'payment_id' => $paymentId,
                'payment_reference' => $result->PaymentReference ?? 'N/A',
            ]);
            ...
            Log::error('Failed to retrieve payment from Exact Online', [
                'connection_id' => $connection->id,
                'payment_id' => $paymentId,
                'error' => $e->getMessage(),
            ]);
