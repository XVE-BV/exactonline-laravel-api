...
class GetJournalsAction
{
    use HandlesExactConnection;

    /**
     * Retrieve journals from Exact Online.
     *
    ...
    public function execute(ExactConnection $connection, array $options = []): Collection
    {
        ...
        try {
            ...
            $this->applyQueryOptions($journal, $options);

            $journals = $journal->get();

            $this->completeRequest($connection, $picqerConnection);

            Log::info('Retrieved journals from Exact Online', [
                'connection_id' => $connection->id,
                'count' => count($journals),
            ]);

            return collect($journals)->map(fn ($j) => $j->attributes());

        } catch (\Exception $e) {
            Log::error('Failed to retrieve journals from Exact Online', [
                'connection_id' => $connection->id,
                'error' => $e->getMessage(),
            ]);
     ...
     */
    protected function applyQueryOptions(Journal $entity, array $options): void
    {
