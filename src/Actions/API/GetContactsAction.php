...
class GetContactsAction
{
    ...
    /**
     * Retrieve contacts from Exact Online.
     *
    ...
    public function execute(ExactConnection $connection, array $options = []): Collection
    {
        ...
        try {
            ...
            $this->applyQueryOptions($contact, $options);

            $contacts = $contact->get();

            $this->completeRequest($connection, $picqerConnection);

            Log::info('Retrieved contacts from Exact Online', [
                'connection_id' => $connection->id,
                'count' => count($contacts),
            ]);

            return collect($contacts)->map(fn ($c) => $c->attributes());

        } catch (\Exception $e) {
            Log::error('Failed to retrieve contacts from Exact Online', [
                'connection_id' => $connection->id,
                'error' => $e->getMessage(),
            ]);
     ...
     */
    protected function applyQueryOptions(Contact $entity, array $options): void
    {
